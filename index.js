...
function artistByCountry(artists, nationality){
  let artistsNationality = [];

  for (let i = 0; i < artists.length; i++) {
    let artist = artists[i];

    if (artist.nationality === nationality) {
      let hasOtherNationalities = false;

      for (let j = 0; j < artist.length; j++) {
        if (i !== j && artists[j].name === artist.name && artists[j].nationality !== nationality) {
          hasOtherNationalites = true;
          break;
        }
      }
      if (!hasOtherNationalities) {
        artistsNationality.push(artist.name);
      }
    }
  }
